...
export const accumulatePost = (userId, postId, type, content) => async (dispatch) =>{
    dispatch({
        type: POST_ACCUMULATE_REQUEST, payload: {userId, postId, type, content}
    });
    try {
        const {data} = await axios.put('/api/forum/accumulatePost', {userId, postId, type, content});
...
// export const reportPost = (userId, postId, type) => async (dispatch) =>{
//     dispatch({
...
export const accumulateComment = (userId, commentId, postId, type, content) => async (dispatch) =>{
    dispatch({
        type: COMMENT_ACCUMULATE_REQUEST, payload: {userId, commentId, postId, type, content}
    });
    try {
        const {data} = await axios.put('/api/forum/accumulateComment', {userId, commentId, postId, type, content});
        dispatch({type: COMMENT_ACCUMULATE_SUCCESSFUL, payload: data});
    } catch (error) {
        dispatch({type: COMMENT_ACCUMULATE_FAILED, 
            payload: error.response 
            && error.response.data.message 
            ? error.response.data.message
            : error.message,});
